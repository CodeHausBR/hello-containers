...
class onda_procob_familiares {
    constructor() {
        ...
        const modelNome = "onda_procob_familiares";
    ...
    async post({data, cpfCnpj}) {
        try {
            ...
            await onda_errors.postNotRes({classe: "onda_procob_familiares", statico: "post", message: JSON.stringify(err)?.slice(0, 4900)});
        }
    }
    ...
    async getOne({documento}) {
        try {
            ...
            await onda_errors.postNotRes({classe: "onda_procob_familiares", statico: "getOne", message: JSON.stringify(err)?.slice(0, 4900)});
...
export default new onda_procob_familiares();
